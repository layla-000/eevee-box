(() => {
  ...
  function globalToolCost(){return Object.entries(globalTools).reduce((sum,[price,count])=>sum+Number(price)*Number(count||0),0);}
  function globalToolCount(){return Object.values(globalTools).reduce((sum,count)=>sum+Number(count||0),0);}
  function syncGlobalToolInputs(){[3000,5000,7000].forEach(price=>{const input=$(`tool${price}`);if(input)input.value=globalTools[price];});const sub=$('globalToolsSubtotal');if(sub)sub.textContent=won(globalToolCost());}
  function render(){
    $('trainingPokemonList').innerHTML=items.map(itemHtml).join('');
    const pokemonRows=items.map((item,i)=>`<div class="training-result-row"><span><b>#${i+1}</b> ${esc(displayName(item,i))}</span><strong>${won(costs(item).total)}</strong></div>`).join('');
    const toolCost=globalToolCost(), toolCount=globalToolCount();
    const toolRow=toolCount?`<div class="training-result-row training-result-tools"><span><b>도구</b> 공용 도구 ${toolCount}개</span><strong>${won(toolCost)}</strong></div>`:'';
    $('trainingResultList').innerHTML=pokemonRows+toolRow;
    const pokemonTotal=items.reduce((s,item)=>s+costs(item).total,0), total=pokemonTotal+toolCost;
    $('grandTotalCost').textContent=won(total);
    $('grandTotalSummary').textContent=`${items.length}마리${toolCount?` · 도구 ${toolCount}개`:''} · 합계 ${won(total)}`;
  ...
  function bind(){
    ...
    $('resetTraining').addEventListener('click',()=>{nextId=1;items=[newItem()];globalTools={3000:0,5000:0,7000:0};render();});
    ...
    document.querySelector('.training-global-tools')?.addEventListener('input',e=>{const price=Number(e.target.dataset.toolPrice);if(!price)return;globalTools[price]=clamp(e.target.value,0,99);render();});
    document.querySelector('.training-global-tools')?.addEventListener('click',e=>{const button=e.target.closest('[data-tool-action="step"]');if(!button)return;const price=Number(button.dataset.toolPrice),delta=Number(button.dataset.delta||0);globalTools[price]=clamp((globalTools[price]||0)+delta,0,99);render();});
  }
